...
if (!isset($_SESSION['id_usuario']) || !isset($_SESSION['rol']) || $_SESSION['rol'] != 1) {
    ...
    echo json_encode(['success' => false, 'message' => 'No autorizado. Solo administradores pueden aprobar solicitudes.']);
    exit;
}
...
try {
    ...
    // Ejecutar la aprobación (sin nota_admin)
    if ($prestamoModelo->aprobarAplazamiento($idSolicitud)) {
        echo json_encode([
            'success' => true,
            'message' => 'Solicitud de aplazamiento aprobada exitosamente'
        ]);
    } else {
        $detalle = '';
    ...
    error_log("Error en aprobar_aplazamiento.php: " . $e->getMessage());
    http_response_code(500);
    // Respuesta con detalle de error TEMPORAL para depuración
    echo json_encode([
        'success' => false,
